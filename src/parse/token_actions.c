...
/*   Updated: 2023/12/13 19:57:11 by wdavey           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t	token_1lt(char **args, t_command *command)
{
	...
	command->fd[FD_IN] = open(args[1], O_RDONLY | O_CLOEXEC);
...
size_t	token_2gt(char **args, t_command *command)
{
	...
	command->fd[FD_OUT] = open(args[1], O_WRONLY | O_CREAT | O_APPEND
			| O_CLOEXEC, 0644);
...
size_t	token_1gt(char **args, t_command *command)
{
	if (STDOUT_FILENO != command->fd[FD_OUT])
		close(command->fd[FD_OUT]);
	command->fd[FD_OUT] = open(args[1], O_WRONLY | O_CREAT | O_CLOEXEC, 0644);
	if (0 > command->fd[FD_OUT])
		return (-1);
	return (1);
}
